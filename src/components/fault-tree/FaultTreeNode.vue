<template>
  ...
  >
    ...
    <g v-if="node.type === 'event-rect'">
      ...
      <!-- 下方逻辑门图形 -->
      <g v-if="node.logicGate && node.logicGate !== ''" :transform="`translate(${node.width / 2}, ${node.height + 10})`">
        <!-- AND门图形 -->
        <g v-if="node.logicGate === 'gate-and'">
          <path
            d="M -15 0 L -15 -10 L 15 -10 L 15 0"
            :fill="selected ? '#e6f7ff' : '#fff'"
            :stroke="selected ? '#1890ff' : '#333'"
            :stroke-width="1.5"
          />
          <path
            d="M -15 0 A 15 10 0 0 0 15 0"
            :fill="selected ? '#e6f7ff' : '#fff'"
            :stroke="selected ? '#1890ff' : '#333'"
            :stroke-width="1.5"
          />
    ...
    <g v-else-if="node.type === 'event-circle'">
      ...
      />
      <path
        :d="`M 0 ${node.height * 0.7} Q ${node.width / 2} ${node.height} ${node.width} ${node.height * 0.7}`"
        :fill="selected ? '#e6f7ff' : '#fff'"
        :stroke="selected ? '#1890ff' : '#333'"
        :stroke-width="selected ? 3 : 2"
      />
